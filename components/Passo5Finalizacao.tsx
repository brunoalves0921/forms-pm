...
export default function Passo5Finalizacao({ etapaAnterior }: Props) {
  ...
  // Referência para o quadro de assinatura e para a impressão
  const sigCanvas = useRef<any>(null);
  const printRef = useRef<HTMLDivElement>(null);

  const dadosAtuais = watch();

  // AQUI ESTÁ A CORREÇÃO: Usando a nova API (contentRef) do react-to-print
  const handlePrint = useReactToPrint({
    contentRef: printRef,
    documentTitle: `Relatorio_Ocorrencia_${dadosAtuais.viatura || 'PMCE'}`,
  });

  // Função disparada no clique do botão verde
  const prepararEImprimir = () => {
    // 1. Pega o desenho do dedo e salva no React Hook Form como Base64 (Imagem)
    if (sigCanvas.current && !sigCanvas.current.isEmpty()) {
      setValue('assinaturaResp', sigCanvas.current.getTrimmedCanvas().toDataURL('image/png'));
    }
    
    // 2. Dá um tempo para o React injetar a imagem no template e chama a impressão
    setTimeout(() => {
        handlePrint();
    }, 300);
  }
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {/* TEMPLATE INVISÍVEL PARA IMPRESSÃO */}
